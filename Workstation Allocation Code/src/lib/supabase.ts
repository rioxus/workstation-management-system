...
export interface WorkstationRequest {
  ...
  status: 'pending' | 'approved' | 'rejected' | 'completed' | 'in_progress' | 'partially_allocated' | 'partially_approved';
  admin_notes?: string;
  approved_by?: string;
  approved_at?: string;
...
export const db = {
  ...
  requests: {
    ...
    approvePartial: async (id: string, approvedBy: string, notes: string) => {
      const { data, error } = await supabase
        .from('workstation_requests')
        .update({
