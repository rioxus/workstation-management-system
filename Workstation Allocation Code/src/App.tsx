...
export default function App() {
  ...
  const [employees, setEmployees] = useState<any[]>([]); // Track all employees for existence checks
  ...
  // Set up real-time subscriptions for automatic data updates
  useEffect(() => {
    if (!selectedRole) return;

    // Subscribe to workstation_requests table changes
    const requestsSubscription = supabase
      .channel('workstation_requests_changes')
      .on(
        'postgres_changes',
        {
          event: '*', // Listen to all events (INSERT, UPDATE, DELETE)
          schema: 'public',
          table: 'workstation_requests'
        },
        (payload) => {
    ...
    // Subscribe to seat_bookings table changes
    const bookingsSubscription = supabase
      .channel('seat_bookings_changes')
      .on(
        'postgres_changes',
        {
          event: '*',
    ...
    // Subscribe to labs table changes (for workstation data updates)
    const labsSubscription = supabase
      .channel('labs_changes')
      .on(
        'postgres_changes',
        {
          event: '*',
    ...
    // Cleanup subscriptions on unmount or role change
    return () => {
      supabase.removeChannel(requestsSubscription);
      supabase.removeChannel(bookingsSubscription);
      supabase.removeChannel(labsSubscription);
    };
  }, [selectedRole]);

  // CRITICAL: Global heartbeat to prevent Supabase free tier from pausing the project
  // Runs even when no one is logged in - as long as ANY browser tab has the app open
  // Supabase pauses projects after 7 days of inactivity - this ensures the database stays active
  // Polling every 6 hours = 4 requests/day × 1 lightweight query = ~4-8 database queries/day
  useEffect(() => {
    // Lightweight heartbeat function - just ping the database to keep it alive
    const heartbeat = async () => {
      try {
        // Simple query to keep database active (doesn't need to load all data)
        await supabase.from('workstation_requests').select('id').limit(1);
        console.log('💓 Database heartbeat: Project kept active');
      } catch (error) {
        console.log('⚠️ Heartbeat query failed (this is normal if database is not set up yet)');
      }
    };
    ...
    // Set up heartbeat polling (every 6 hours)
    const heartbeatInterval = setInterval(() => {
      console.log('💓 Running database heartbeat...');
      heartbeat();
  ...
  }, []); // Empty dependency array = runs once when app mounts, stops when app unmounts
  ...
  const loadDataFromSupabase = async () => {
    try {
      ...
      const [requestsData, stats, workstationMapData, seatBookingsData, employeesData] = await Promise.all([
        ...
        dataService.getEmployees().catch(() => []) // Gracefully handle if employees don't exist yet
      ]);
    ...
    try {
      ...
      if (formData.seats && formData.labName) {
        ...
        const requestData = {
          ...
          preferredFloor: formData.preferredFloor || '', // Add preferred floor field
          justification: formData.justification || 'Seat booking request',
          remarks: formData.remarks || '', // Manager's remarks
          requestedAllocationDate: formData.requestedAllocationDate || '', // Requested allocation date
        };
  ...
  const handleReject = async (requestId: string, reason: string) => {
    // Add debounce check - prevent duplicate submissions
    const debounceKey = `reject-${requestId}`;
    const lastSubmit = (window as any)[debounceKey];
    const now = Date.now();
    
    if (lastSubmit && now - lastSubmit < 3000) {
      // Already submitted within last 3 seconds, ignore duplicate
      console.log('⚠️ Duplicate reject request detected, ignoring');
      return;
    }
    ...
    try {
      toast.loading('Rejecting request...', { id: 'reject-request' });
      await dataService.rejectRequest(requestId, reason);
      
      // Reload data
      await loadDataFromSupabase();
      
      toast.success('Request rejected', { id: 'reject-request' });
    } catch (error) {
      console.error('Error rejecting request:', error);
      toast.error('Failed to reject request. Please try again.', { id: 'reject-request' });
    } finally {
  ...
  const handleFinalApprove = async (requestId: string, allAllocations: any[], partialReason: string | null = null) => {
    try {
      ...
      // Step 3: Check if this is a partial allocation or full allocation
      if (partialReason) {
        // PARTIAL ALLOCATION: Approve allocated seats and reject remaining
        const totalRequired = request.numWorkstations || request.numEmployees;
        const totalAllocated = allAllocations.reduce((sum: number, alloc: any) => sum + alloc.seats.length, 0);
        const remainingSeats = totalRequired - totalAllocated;

        console.log(`🟡 Partial Allocation: ${totalAllocated} allocated, ${remainingSeats} rejected`);

        await dataService.approvePartialAllocation(
          requestId,
          currentUserInfo.employeeId,
          allAllocations,
          `Partial allocation: ${totalAllocated} of ${totalRequired} seats allocated. Remaining ${remainingSeats} seats could not be allocated. Reason: ${partialReason}`,
          remainingSeats,
          partialReason
        );
      } else {
        // FULL ALLOCATION: Standard approval
      ...
      if (partialReason) {
        toast.success('✅ Partial allocation approved! Manager and HOD have been notified.');
      } else {
  ...
  // Calculate pending count based on user role
  // CRITICAL SECURITY FIX: Managers should ONLY see their own pending requests in the badge
  // AUDIT TRAIL FIX: Exclude pending requests from deleted employees (employee no longer exists in system)
  const pendingCount = (() => {
    // Get pending requests based on role
    const pendingRequests = selectedRole === 'Manager' 
      ? requests.filter(r => r.status === 'pending' && r.requestorId === userInfo.employeeId)
      : requests.filter(r => r.status === 'pending');
    
    // Filter out requests where employee no longer exists
    const validPendingRequests = pendingRequests.filter(request => {
      return employees.some(emp => emp.employee_id === request.requestorId);
    });
    
    return validPendingRequests.length;
  })();
            ...
            <div className="flex items-center gap-3">
              <h2 className="text-sm md:text-base text-slate-600">
                {activePage === 'dashboard' ? 'Dashboard' : 
                 activePage === 'requests' ? (userInfo?.role === 'admin' ? 'System Requests' : 'My Requests') :
