...
export function RequestForm({ onSubmit, onCancel, userInfo }: RequestFormProps) {
  const [formData, setFormData] = useState({
    numWorkstations: '',
    location: '',
    preferredFloor: '', // Add preferred floor field
    division: '',
  ...
  const loadFloors = async () => {
    try {
      // Fetch all labs to get unique floor names
      const labs = await db.labs.getAll();
      const uniqueFloors = Array.from(new Set(
        labs
          .map((lab: any) => lab.floors?.floor_name)
          .filter(Boolean) // Remove undefined/null values
      )).sort();
      setFloors(uniqueFloors);
    } catch (error) {
      console.error('Error loading floors:', error);
      // Fallback to default floors if DB fetch fails
      setFloors(['Ground Floor', 'First Floor', 'Second Floor']);
    }
  };
      ...
      <Card>
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Location, Floor, and Requested Allocation Date */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleChange('preferredFloor', value)}
                >
                  <SelectTrigger>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Optional: Select your preferred floor
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0 bg-white border border-slate-200 shadow-md rounded-lg" align="start" sideOffset={4}>
                    <CustomCalendar
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Select when you want workstations allocated
                </p>
              </div>
            </div>
