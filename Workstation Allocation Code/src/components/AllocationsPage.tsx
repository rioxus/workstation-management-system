...
import { User, Calendar, Building, ArrowLeft, Save, CheckCircle, Edit2, Trash2, X, Layers, AlertTriangle } from 'lucide-react';
...
interface AllocationsPageProps {
  ...
  onFinalApprove: (requestId: string, allAllocations: any[], partialReason?: string) => void;
}
...
export function AllocationsPage({ request, onBack, onSaveAllocation, onFinalApprove }: AllocationsPageProps) {
  ...
  const [labs, setLabs] = useState<any[]>([]); // Add labs state
  ...
  // Partial allocation dialog state
  const [showPartialAllocationDialog, setShowPartialAllocationDialog] = useState(false);
  const [partialAllocationReason, setPartialAllocationReason] = useState('');
  ...
  const loadWorkstationData = async () => {
    try {
      ...
      // Set labs state for use in getAssetIdsForPositions
      setLabs(labs);
  ...
  // Helper function to get Asset IDs for selected positions
  const getAssetIdsForPositions = (positions: number[]): string[] => {
    if (!currentAllocationDetails || positions.length === 0) {
      return [];
    }
    
    // Look up asset range from labs table instead of labAssetRanges table
    const labRecord = labs.find(
      lab => lab.id === currentAllocationDetails.labId
    );
  ...
  const handleBookingSubmit = (bookingData: any) => {
    ...
    // NEW LOGIC: Filter out already-saved seats in the same lab
    // This ensures we only show NEWLY selected seats, not the ones already saved
    const selectedSeats = bookingData.selectedSeats || [];
    
    // Check if there's an existing allocation in the same lab
    const existingAllocation = savedAllocations.find(
      (alloc: any) => 
        alloc.labId === bookingData.labId && 
        alloc.floorId === bookingData.floorId
    );
    
    // If we're in edit mode, show all selected seats (user is editing)
    const isInEditMode = editModeInitialSeats.length > 0;
    
    let filteredSeats = selectedSeats;
    
    if (existingAllocation && !isInEditMode) {
      // MERGE MODE: Filter out already-saved seats, show only NEW selections
      filteredSeats = selectedSeats.filter((seat: number) => !existingAllocation.seats.includes(seat));
      console.log('🔄 Filtering seats - Existing:', existingAllocation.seats, 'Selected:', selectedSeats, 'Filtered (new only):', filteredSeats);
    }
    
    // Store the FILTERED seats (only new selections) for display
    // Asset IDs will be auto-derived from positions when saving
    setSelectedSeatsFromGrid(filteredSeats);
    
    // Store the FULL selection in currentAllocationDetails for merge logic in handleSave
    setCurrentAllocationDetails({
      ...bookingData,
      fullSelectedSeats: selectedSeats, // Store full selection for grid state
      filteredSeats: filteredSeats // Store filtered seats for display
    });
  };
  ...
  const handleSave = async () => {
    ...
    let assetIds: number[] = derivedAssetIds.map(id => parseInt(id, 10));
    ...
    try {
      ...
      if (isExistingAllocation && !isInEditMode) {
        // MERGE MODE: Add ONLY the newly selected seats to existing allocation
        // selectedSeatsFromGrid already contains ONLY the new seats (filtered in handleBookingSubmit)
      ...
      if (isEditMode) {
        // EDIT MODE: Delete old seats that are no longer selected, then update/create new seats
        
        // Step 1: Find all existing bookings for this allocation (the old seats)
        const oldBookingsInThisAllocation = allExistingBookings.filter(
          (booking: any) =>
            ...
            booking.status === 'pending' &&
            editModeInitialSeats.includes(booking.seat_number)
        );

        // Step 2: Delete bookings that are NO LONGER in the new selection
        // Example: If old seats were [37, 41, 42, 43] and new selection is [37, 42], delete [41, 43]
        const seatsToDelete = oldBookingsInThisAllocation.filter(
          (booking: any) => !selectedSeatsFromGrid.includes(booking.seat_number)
        );

        for (const bookingToDelete of seatsToDelete) {
          await db.seatBookings.delete(bookingToDelete.id);
        }

        // Step 3: Update or create bookings for the new selection
        for (let i = 0; i < selectedSeatsFromGrid.length; i++) {
          ...
          if (existingBooking) {
            ...
            // Create new record for this seat (wasn't in the old selection)
        ...
        // Delete existing pending records to prevent duplicates
  ...
  const handleSaveAndApprove = async () => {
    ...
    // NEW: Check if this is a partial allocation
    if (currentTotal < totalRequired) {
      // PARTIAL ALLOCATION: Show reason dialog
      setShowPartialAllocationDialog(true);
      return;
    }

    // FULL ALLOCATION: Proceed normally
    onFinalApprove(request.id, allAllocations, null);
  };

  // Handle partial allocation confirmation
  const handleConfirmPartialAllocation = () => {
    if (partialAllocationReason.trim() === '') {
      toast.error('Please provide a reason for partial allocation');
      return;
    }
    ...
    // Call onFinalApprove with the partial allocation reason
    onFinalApprove(request.id, allAllocations, partialAllocationReason);
    
    // Close dialog and reset
    setShowPartialAllocationDialog(false);
    setPartialAllocationReason('');
  };
  ...
  const handleCancelSelection = async () => {
    ...
    if (isInEditMode) {
      ...
      // Silent clear - no toast notification
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {request.preferredFloor && (
              <div>
                <Label className="text-blue-800">Preferred Floor</Label>
                <div className="flex items-center gap-2 mt-1">
                  <Layers className="w-4 h-4 text-blue-600" />
                  <span className="font-medium">{request.preferredFloor}</span>
                </div>
              </div>
            )}
            <div>
      ...
      {/* Partial Allocation Confirmation Dialog */}
      <Dialog open={showPartialAllocationDialog} onOpenChange={setShowPartialAllocationDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Partial Allocation - Provide Reason</DialogTitle>
            <DialogDescription>
              You are submitting a partial allocation. Please provide a reason for why the remaining seats could not be allocated.
            </DialogDescription>
          </DialogHeader>
          ...
          <Alert className="bg-amber-50 border-amber-200">
            ...
            <AlertDescription className="text-amber-800 text-sm">
              <div className="space-y-1">
                <p><strong>Total Seats Requested:</strong> {totalRequired}</p>
                <p className="text-green-700"><strong>Seats Allocated:</strong> {totalAllocatedSeats}</p>
                <p className="text-red-700"><strong>Seats Remaining:</strong> {totalRequired - totalAllocatedSeats}</p>
              </div>
            </AlertDescription>
          </Alert>

          <div className="space-y-2">
            <Label htmlFor="partial-reason" className="text-sm font-semibold">
              Reason for Partial Allocation <span className="text-red-600">*</span>
            </Label>
            <Textarea
              id="partial-reason"
              value={partialAllocationReason}
              onChange={(e) => setPartialAllocationReason(e.target.value)}
              placeholder="Explain why the remaining seats could not be allocated (e.g., insufficient lab capacity, preferred floor not available, etc.)"
              className={partialAllocationReason.trim() === '' ? 'border-red-300' : ''}
              rows={4}
            />
            {partialAllocationReason.trim() === '' && (
              <p className="text-xs text-red-600 flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" />
                Reason is required for partial allocation
              </p>
            )}
          </div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleConfirmPartialAllocation}
              disabled={partialAllocationReason.trim() === ''}
              className="bg-amber-600 hover:bg-amber-700"
            >
              Submit Partial Allocation
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
