...
export function UserDashboard({ requests, userInfo, onNavigateToRequests, dashboardStats, onAddRequest, seatBookings = [], onRefreshData, onNavigateToAddRequest }: UserDashboardProps) {
  ...
  // Filter requests for current user by Employee ID ONLY
  // CRITICAL SECURITY FIX: Managers should ONLY see their own requests, not other managers' requests in the same division
  // Each manager's requests are strictly confidential and isolated by Employee ID
  const userRequests = requests.filter(req => {
    // Filter by Employee ID - this ensures complete data isolation
          ...
          {selectedRequest && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-lg">
                ...
                {/* Show Preferred Floor if available */}
                {selectedRequest.preferredFloor && selectedRequest.preferredFloor.trim() !== '' && (
                  <div>
                    <p className="text-xs text-slate-500 mb-1">Preferred Floor</p>
                    <p className="text-sm font-medium text-blue-600">{selectedRequest.preferredFloor}</p>
                  </div>
                )}
                ...
                {/* Show Floor for approved/partially approved/partially allocated */}
                {(selectedRequest.status === 'approved' || selectedRequest.status === 'partially_approved' || selectedRequest.status === 'partially_allocated') && selectedRequest.floor && selectedRequest.floor.trim() !== '' && (
                  <div>
                    <p className="text-xs text-slate-500 mb-1">Floor</p>
                    <p className="text-sm font-medium text-blue-600">{selectedRequest.floor}</p>
                  </div>
                )}
              </div>
              ...
              {/* Partially Approved - Show Admin Notes (Partial Allocation Reason) */}
              {selectedRequest.status === 'partially_approved' && selectedRequest.adminNotes && (() => {
                // Extract just the reason from admin notes
                const reasonMatch = selectedRequest.adminNotes.match(/Reason:\s*(.+)$/);
                const reason = reasonMatch ? reasonMatch[1].trim() : selectedRequest.adminNotes;
