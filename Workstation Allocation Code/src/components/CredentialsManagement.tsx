...
interface Employee {
  ...
  division?: string;
  password?: string;
  hod_email?: string; // HOD email for managers
  created_at: string;
  updated_at: string;
}
...
export function CredentialsManagement({ onDataChange }: CredentialsManagementProps) {
  ...
  const handleSubmit = async () => {
    ...
    // For Managers, HOD email is mandatory
    if (formData.role === 'Manager' && !formData.hod_email.trim()) {
      toast.error('HOD Email is required for Managers');
      return;
    }

    // Validate HOD email format if provided
    if (formData.role === 'Manager' && formData.hod_email.trim() && !formData.hod_email.includes('@')) {
      toast.error('Please enter a valid HOD email address');
      return;
    }
    ...
    try {
      ...
      if (editMode && selectedEmployee) {
        ...
        await db.employees.update(selectedEmployee.id, {
          ...
          hod_email: formData.role === 'Manager' ? formData.hod_email.trim() || undefined : undefined,
        });
  ...
  const handleDelete = async () => {
    ...
    try {
      console.log(`Deleting employee ${selectedEmployee.employee_id}...`);
      
      // Step 1: Delete only PARTIAL seat allocations (unreleased seats)
      // Approved assignments remain in Workstation Data
      try {
        const { error: bookingsError } = await supabase
          .from('seat_bookings')
          .delete()
          .eq('requestor_id', selectedEmployee.employee_id) // FIXED: Use requestor_id instead of requestor_employee_id
          .eq('status', 'pending'); // FIXED: Delete only pending bookings (partial allocations)
        
        if (bookingsError) {
          console.warn('⚠️ Error deleting partial seat bookings:', bookingsError);
        } else {
          console.log('✅ Deleted partial seat allocations (unreleased seats)');
        }
      } catch (err) {
        console.warn('Could not delete partial bookings:', err);
      }

      // Step 2: Delete the employee profile (revokes login access)
      // Note: Approved assignments stay in database
      // Note: Request history stays in workstation_requests table
      await db.employees.delete(selectedEmployee.id);
      
      toast.success(
        `Employee profile deleted. Login access revoked. Approved assignments and request history preserved in system.`,
        { duration: 5000 }
      );
                  ...
                  {employees.map((employee) => (
                    <TableRow key={employee.id}>
                      ...
                      <TableCell className="text-sm text-slate-500">
                        {employee.hod_email || <span className="text-slate-400 italic">—</span>}
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            ...
            {formData.role === 'Manager' && (
              <div className="space-y-2">
                <Label htmlFor="hod_email">
                  HOD Email
                  <span className="text-xs text-slate-500 ml-2">
                    (Optional - emails sent to HOD if provided)
                  </span>
                </Label>
                <Input
                  id="hod_email"
                ...
                />
                {formData.hod_email && !formData.hod_email.includes('@') && (
            ...
            <div className="space-y-3">
              ...
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <ul className="text-sm text-blue-900 space-y-1 list-disc list-inside">
                  <li>Login access will be <strong>immediately removed</strong></li>
                  <li>Partial allocations will be <strong>deleted</strong> (unreleased seats freed up)</li>
                  <li>Approved seat assignments will <strong>remain in Workstation Data</strong></li>
                  <li>Request history will be <strong>preserved</strong> in System Requests tab</li>
                </ul>
              </div>

              <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                <p className="text-sm text-green-900">
                  <strong>✓ Data Preservation:</strong> All approved assignments remain visible in the Allocations tab. Use "Clean Data" in System Requests to remove request history later if needed.
                </p>
              </div>
            </div>
          )}
