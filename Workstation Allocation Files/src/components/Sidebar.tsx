...
import { Home, FileText, Plus, CheckSquare, MapPin, Bell, LogOut, BarChart3, Wrench, Calendar, Settings, Menu, X, UserCog } from 'lucide-react';
...
export function Sidebar({ activePage, onNavigate, userRole, userName, pendingCount = 0, onLogout }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { id: 'requests', label: 'System Requests', icon: FileText, roles: ['admin', 'manager', 'technical'], badge: true },
    { id: 'allocations', label: 'Allocations', icon: CheckSquare, roles: ['admin'] },
    { id: 'workstation-management', label: 'Workstation Data', icon: Settings, roles: ['admin'] },
    { id: 'credentials', label: 'Credentials', icon: UserCog, roles: ['admin'] },
